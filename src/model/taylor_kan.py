...
class TaylorKANLinear(torch.nn.Module):
    def __init__(
        ...
        参数:
            ...
            drop_rate (float): 随机丢弃概率。
            drop_scale (bool): 是否使用缩放的随机丢弃概率。
        """
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.training and self.drop_rate > 0:
            mask = torch.empty(taylor_output.shape, device=taylor_output.device).bernoulli_(1 - self.drop_rate)
            if self.drop_scale:
                taylor_output = taylor_output * mask / (1 - self.drop_rate)
            else:
                taylor_output = taylor_output * mask
