...
class DropKANLinear(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.training and self.drop_rate > 0:
            mask = torch.empty(spline_output.shape, device=spline_output.device).bernoulli_(1 - self.drop_rate)
            if self.drop_scale:
                spline_output = spline_output * mask / (1 - self.drop_rate)
            else:
        ...
        # if self.training and self.drop_rate > 0:
        #     mask = torch.empty(output.shape, device=output.device).bernoulli_(1 - self.drop_rate)
        #     if self.drop_scale:
        #         output = output * mask / (1 - self.drop_rate)
        #     else:
        #         output = output * mask
